...
enduro.templating_engine.registerHelper('practices', function (options) {
    ...
    return enduro.api.pagelist_generator.get_cms_list()
        .then((pagelist) => {
            ...
            practice_entries = _.chain(pagelist.structured.practices)
            ...
            for (page_id in practice_entries) {
                ...
                function get_content (page) {
                    get_content_promises.push(enduro.api.flat.load(page.fullpath).then((content) => { page.practice_entry = content }))
                }
        ...
        .then(() => {
            practice_entries = practice_entries.filter(entry => entry.hidden !== true);
